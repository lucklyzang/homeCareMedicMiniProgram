<template>
	<view class="content-box">
		...
		<view class="top-area-box">
			<view class="nav">
				<nav-bar :home="false" backState='3000' bgColor="none" :title="`${medicalCareAptitudeMessage['name']}认证`" @backClick="backTo">
				</nav-bar> 
		  </view>
		</view>
		<view class="real-name-authentication-box">
			<view class="real-name-authentication-step">
				...
				<view class="real-name-authentication-step-bottom">
					<view class="step-left" :class="{'stepStyle' : stepActive === 0 || stepActive === 1 }">{{ `${medicalCareAptitudeMessage['name']}认证` }}</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			console.log('其它',this.medicalCareAptitudeMessage);
		},
		methods: {
			...mapMutations([
			]),
